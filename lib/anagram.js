...
function findAnagrams(rack, userOptions, callback) {
	...
	var uniqueLetters = findUniqueLetters(rack),
		...
		//groupedMatches = { 'words': [], 'count': 0 }, // grouped
		groupedMatches = {},
		meta = { count: 0, input: '', groups: [] },
		debug = false,
		debugTrail = false,
		recurse = true,
		i = 0,
    ...
    try {
			...
			for ( var node in cur ) {
				...
				if ( freq.total > 0 && ( freq[node] !== undefined || ( isWildcard && node !== '$' ) ) ) {
					...
					if (val === 0 || val.$ === 0) {
			            ...
			            // Limit
			            // Skip
						//groupedMatches.words.push({ length: localTrailLength, word: localTrail.replace(/<\/?em>|\*/g, ''), wordFormat: localTrail }); // grouped
						if (groupedMatches[localTrailLength] === undefined) groupedMatches[localTrailLength] = [];
						groupedMatches[localTrailLength].push({ word: localTrail.replace(/<\/?em>|\*/g, ''), wordFormat: localTrail });
						meta.count++;
		...
		for (var g in groupedMatches) {
			meta.groups.push({
				field: 'length',
				value: g,
				items: groupedMatches[g],
				hasSubgroups: false,
				aggregates:{}
			});
		}
